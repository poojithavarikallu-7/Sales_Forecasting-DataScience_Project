...
# Re-loading data (assuming 'Train.csv' is available)
df = pd.read_csv('Train.csv')

# --- Data Cleaning and Preprocessing (Consolidated) ---
...
# Fill 0 values in Item_Visibility with mean
df.loc[:,'Item_Visibility'].replace([0],[df['Item_Visibility'].mean()],inplace = True)

# Correct Item_Fat_Content typos
df['Item_Fat_Content'] = df['Item_Fat_Content'].replace({'LF' : 'Low Fat', 'low fat' : 'Low Fat', 'reg' : 'Regular'})

# Add New_Item_Type column
df['New_Item_Type'] = df['Item_Identifier'].apply(lambda x : x[:2])
df['New_Item_Type'] = df['New_Item_Type'].replace({'FD' : 'Food', 'NC' : 'Non-Consumables', 'DR' : 'Drinks'})

# Mark Non-Consumables as Non-Edible fat content
df.loc[df['New_Item_Type'] == 'Non-Consumables','Item_Fat_Content'] = 'Non-Edible'

# Add Outlet_Years column
curr_time = datetime.datetime.now()
df['Outlet_Years'] = df['Outlet_Establishment_Year'].apply(lambda x: curr_time.year - x)

# Label Encoding Outlet_Identifier
le = LabelEncoder()
df['Outlet'] = le.fit_transform(df['Outlet_Identifier'])
...
print("Data preparation and splitting complete. x_train, x_test, y_train, y_test are now defined.")
...
# --------------------------------------------------------------
# MODEL COMPARISON TABLE + BEST MODEL IN TEXT FORMAT
# --------------------------------------------------------------
...
# --------------------------------------------------------------
# PRINT TABLE IN YOUR REQUIRED TEXT FORMAT
# --------------------------------------------------------------
print("\n\n==================== MODEL COMPARISON TABLE ====================\n")
print("| {:20} | {:8} | {:8} | {:21} |".format("Model Name", "Train R²", "Test R²", "Cross-Validation R²"))
print("|----------------------|----------|----------|-----------------------|")

for row in results:
    print("| {:20} | {:8.2f} | {:8.2f} | {:21.2f} |".format(row[0], row[1], row[2], row[3]))


# --------------------------------------------------------------
# FIND BEST MODEL (BASED ON TEST R²)
# --------------------------------------------------------------
best_row = max(results, key=lambda x: x[2])   # x[2] = Test R²
best_model_name = best_row[0]
best_model_test_r2 = best_row[2]

print("\n\n===================== BEST MODEL =====================\n")
print(f"Best Model: {best_model_name}")
print(f"Best Test R² Score: {best_model_test_r2:.2f}")
